...
export default function QuickSettings(monitor: Gdk.Monitor) {
    ...
    audioOutputChooser.connect("changed", (self) => {
        wp?.audio.speakers.find((e)=>{return e.id.toString() == self.activeId})?.set_is_default(true)
    })
    return <window
        name="quicksettings"
                ...
                <button className="settings-entry"
                    ...
                    setup={(self) => {
                        network.wifi.connect("state-changed", (wifi, newState) => {
                            self.className = (newState == 100) ? "settings-entry-active" : "settings-entry"
                        })
                        self.className = (network.wifi.state == 100) ? "settings-entry-active" : "settings-entry"
                    }}
                >
                    <box spacing={4}>
                        <box vertical={true} spacing={4}>
                            ...
                            <label
                                ...
                                setup={(self) => {
                                    let updateState = (wifi: AstalNetwork.Wifi, newState: AstalNetwork.DeviceState) => {
                ...
                <button className="settings-entry">
                    <box spacing={4}>
                        <box vertical={true} spacing={4}>
                            ...
                            <label className="settings-entry-status" label="已启用" ellipsize={Pango.EllipsizeMode.END} />
                        </box>
                        <box hexpand={true} />
                        <Gtk.Separator visible={true} />
                        <icon icon="go-next-symbolic" />
                ...
                <button className="settings-entry">
                    <box spacing={4}>
                        <box vertical={true} spacing={4}>
                            ...
                            <label className="settings-entry-status" label="正在刷新..." ellipsize={Pango.EllipsizeMode.END} />
                        </box>
                        <box hexpand={true} />
                        <Gtk.Separator visible={true} />
                        <icon icon="go-next-symbolic" />
