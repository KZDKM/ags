...
export function GetAppInfo(appID) {
  return appList.find((info) => {
    return (
      info
        .get_id()
        ?.substring(0, info.get_id()?.lastIndexOf("."))
        .toLowerCase() == appID.toLowerCase() ||
      info
        .get_id()
        ?.substring(0, info.get_id()?.lastIndexOf("."))
        .toLowerCase() ==
        appID.substring(appID.lastIndexOf(".") + 1).toLowerCase() ||
      info
        .get_id()
        ?.substring(0, info.get_id()?.lastIndexOf("."))
        .substring(0, info.get_id()?.indexOf("-")) == appID ||
      info.get_startup_wm_class() == appID ||
      (info.get_executable() == appID && info.get_executable() != "steam") ||
      (info.get_string("Icon")?.replace("steam_icon_", "") ==
        appID.replace("steam_app_", "") &&
        info.get_string("Icon") != "steam")
    );
  });
}
